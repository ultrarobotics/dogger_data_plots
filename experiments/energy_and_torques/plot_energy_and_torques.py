...
motor_indices_to_plot = [5]  # Motors 3, 6, 9, 12 if 1-based
...
cumulative_plot_path = "plots/cumulative_energy_consumption.pdf"
...
# -----------------------------------
# Plot 2: Motor Efforts for Selected Runs and Timeframe
# -----------------------------------

# Define figure size parameters
figsize_motor = (textwidth, textwidth * 0.26)  # Increase height for multiple subplots

# Create a figure for Motor Efforts
fig_motor, axes_motor = plt.subplots(
    len(motor_indices_to_plot), 1, figsize=figsize_motor, sharex=True
)
if len(motor_indices_to_plot) == 1:
    axes_motor = [axes_motor]  # Ensure it's iterable

# Define selection parameters
selected_run_number = 1  # Choose the run to plot (1-based index). For Run 1, set to 1.
time_frame_start = 70  # Start time in seconds
time_frame_end = 72.0  # End time in seconds

# Validate the timeframe
if time_frame_start >= time_frame_end:
    raise ValueError("time_frame_start must be less than time_frame_end.")

for idx, motor_num in enumerate(motor_indices_to_plot):
    ax_motor = axes_motor[idx]
    ...
    for file_runs in all_runs:
        ...
        # Check if the selected run exists for this file
        if len(file_runs["runs"]) < selected_run_number:
            print(
                f"File '{label}' does not have Run {selected_run_number}. Skipping.",
                file=sys.stderr,
            )
            continue

        # Retrieve the selected run (1-based index)
        run = file_runs["runs"][selected_run_number - 1]

        rel_time = np.array(run["relative_time"])
        motor_effort = np.array([me[motor_num] for me in run["motor_efforts"]])

        # Create a mask for the specified timeframe
        mask = (rel_time >= time_frame_start) & (rel_time <= time_frame_end)

        # Check if there's any data within the specified timeframe
        if not np.any(mask):
            print(
                f"No data within {time_frame_start}-{time_frame_end} sec for '{label}' Run {selected_run_number}. Skipping this run in the plot.",
                file=sys.stderr,
            )
            continue

        # Plot only the data within the specified timeframe
        ax_motor.plot(
            rel_time[mask],
            motor_effort[mask],
            label=label,
    ...
    # Add a thick y=0 line for reference
    ax_motor.axhline(y=0, color="black", linewidth=1.5, linestyle="--")

    # Set y-axis limits based on your data range or keep as is
    ax_motor.set_ylim(-3.5, 3.5)

    # Set titles and labels
    # ax_motor.set_title(f'Motor Knee {["FL", "FR", "RL", "RR"][idx]} Effort Over Time ({time_frame_start}-{time_frame_end}~s)')
    ax_motor.set_ylabel(r"Motor Effort [Nm]")

    # Customize grid and spines
    ax_motor.grid(True, linestyle="--", alpha=0.5)
    ax_motor.spines["top"].set_visible(False)
    ax_motor.spines["right"].set_visible(False)

    # Add legend with appropriate font size
    ax_motor.legend()

# Set the x-label for the bottom subplot
axes_motor[-1].set_xlabel(r"Time [s]")

# Adjust layout for better spacing
plt.tight_layout()

# Save the motor efforts plot
motor_plot_path = "plots/motor_efforts.pdf"
os.makedirs(os.path.dirname(motor_plot_path), exist_ok=True)
save_plot(fig_motor, motor_plot_path)
print(f"Motor efforts plot saved to {motor_plot_path}")
